...
export class StageCreator {
    ...
    private async createDoubleEliminationSkipFirstRound(
        ...
        const { losers: losersWb, winner: winnerWb } = await this.createStandardBracket(
            stageId,
            1,
            directInWb
        )
    ...
    private async getRoundRobinGroups(): Promise<ParticipantSlot[][]> {
        ...
        const groupSizes = this.stage.settings?.groupSizes
        if (Array.isArray(groupSizes)) {
            return helpers.makeGroupsBySizes(ordered, groupSizes)
        }
